...
export class MathsPage {
  ...
  levels: Level[] = [
    ...
    { number: 4, attempted: false },
    { number: 5, attempted: false },
    { number: 6, attempted: false },
    { number: 7, attempted: false }
  ];
  ...
  loadScores() {
    this.levels.forEach(level => {
      const key = `maths-level-${level.number}`;
  ...
  takeTest(level: Level) {
    ...
    this.router.navigate(['/quiz', 'maths', level.number]);
    this.closeModal();
  }
  goToTabs2() {
  this.router.navigate(['/tabs/tab2']); // replace with your actual route for tabs2
}
